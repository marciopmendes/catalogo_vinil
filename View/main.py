...
#Criação dos botões na tela principal
    ...
    def alterarDiscoButton(self):
        alterar_disco_button = tk.Button(master=self.main_window, text="Alterar Cadastro do Disco", width=30, height=1)
        alterar_disco_button.grid(row=3, column=0, padx=4, pady=4)
    
    def excluirDiscoButton(self):
        excluir_disco_button = tk.Button(master=self.main_window, text="Excluir Disco", width=30, height=1)
        excluir_disco_button.grid(row=4, column=0, padx=4, pady=4, columnspan=3)
        
    def cadastrarMusicaButton(self):
        cadastrar_musica_button = tk.Button(master=self.main_window, text="Cadastrar Músicas", width=30, height=1)
        cadastrar_musica_button.grid(row=5, column=0, padx=4, pady=4, columnspan=3)
        
    def pesquisarMusicaButton(self):
        pesquisar_musica_button = tk.Button(master=self.main_window, text="Pesquisar Música", width=30, height=1)
        pesquisar_musica_button.grid(row=6, column=0, padx=4, pady=4)
        
    def alterarMusicaButton(self):
        alterar_musica_button = tk.Button(master=self.main_window, text="Alterar Cadastro da Música", width=30, height=1)
        alterar_musica_button.grid(row=7, column=0, padx=4, pady=4)
    
    def excluirMusicaButton(self):
        excluir_musica_button = tk.Button(master=self.main_window, text="Excluir Música", width=30, height=1)
